...
const imgs = [
  "/assets/banner/sale-banner-5.webp",
  "/assets/banner/sale-banner-5.webp",
  "/assets/banner/sale-banner-5.webp",
  "/assets/banner/sale-banner-5.webp",
  "/assets/banner/sale-banner-5.webp",
      ...
      {imgs.map((imgSrc, idx) => {
        return (
          <motion.div
            ...
            transition={SPRING_OPTIONS}
            className="aspect-video shrink-0 rounded-xl bg-transparent object-contain w-screen"
          />
        );
      })}
    </>
...
const Dots = ({
  ...
  return (
    <div className="mt-0 md:mt-4 flex w-full justify-center gap-2">
      {imgs.map((_, idx) => {
        return (
          <button
            key={idx}
            onClick={() => setImgIndex(idx)}
            className={`h-3 w-3 rounded-full transition-colors ${idx === imgIndex ? "bg-gray-500" : "bg-gray-300"
              }`}
          />
        );
      })}
